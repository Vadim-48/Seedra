...
export async function initCardFilter() {
    const inputFilterFormWrap = document.querySelectorAll("[data-cards-filter]");
    ...
    inputFilterFormWrap.forEach((form) => {
        ...
        priceSlider.noUiSlider.on('change', async function () {
            await filterFunction(form);
            syncFormFilters(form);
        })
    ...
    function syncFormFilters(form) {
        const checkedInputList = Array.from(form.querySelectorAll("input:checked")).map(el => el.value);
        const priceInputList = Array.from(form.querySelectorAll("input[type='number']")).map(el => el.value);

        // console.log(priceInputList);
        inputFilterFormWrap.forEach((otherForm) => {
            if (otherForm !== form) {
                const checkboxOtherList = otherForm.querySelectorAll("input[type='checkbox']");
                const priceOtherValues = otherForm.querySelectorAll("input[type='number']");
                const priceValues = priceInputList.map(Number);

                checkboxOtherList.forEach(input => {
                    if (checkedInputList.includes(input.value)) {
                        input.checked = true;
                    } else {
                        input.checked = false;
                    }
                })
                for (let i = 0; i < priceOtherValues.length; i++) {
                    priceOtherValues[i].value = priceInputList[i];
                }

                const otherSlider = otherForm.querySelector(".filter-form__price-slider");
                if (otherSlider && otherSlider.noUiSlider) {
                    otherSlider.noUiSlider.set(priceValues);
                }
            }
        })
    }

    async function filterFunction(form) {
        cardsWrap.innerHTML = '';
